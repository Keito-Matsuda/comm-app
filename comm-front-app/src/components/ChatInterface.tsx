...
// 型定義
type AgentId = 'user' | 'muscle' | 'gloomy';
...
export default function ChatInterface() {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      // 3. レスポンスデータの抽出
      const muscleText =
        data.steps?.['muscle-reply']?.output?.muscleResponse ||
        "（筋肉男からの応答が取得できませんでした）";

      const gloomyText =
        data.result?.gloomyResponse || // 最終結果に入っている場合
        data.steps?.['gloomy-reply']?.output?.gloomyResponse || // ステップ出力に入っている場合
        "（根暗女からの応答が取得できませんでした）";

      // 4. エージェントの変身を表示
      // -- 筋肉男のターン --
      const muscleMsg: Message = {
        id: Date.now().toString() + '-m',
        agentId: 'muscle',
        name: AGENTS.muscle.name,
        content: muscleText,
        timestamp: new Date(),
      };

      setTimeout(() => {
        setMessages((prev) => [...prev, muscleMsg]);

        // -- 根暗女のターン --
        setTimeout(() => {
          const gloomyMsg: Message = {
            id: Date.now().toString() + '-g',
            agentId: 'gloomy',
            name: AGENTS.gloomy.name,
            content: gloomyText,
            timestamp: new Date(),
          };
          setMessages((prev) => [...prev, gloomyMsg]);
      ...
      <header className="p-4 border-b border-zinc-200 dark:border-zinc-800 flex items-center gap-4 bg-white/90 dark:bg-zinc-900/90 backdrop-blur-sm z-10 sticky top-0">
        ...
        <div className="flex gap-2">
          ...
          <span className="text-xs font-medium bg-red-100 text-red-800 px-2.5 py-0.5 rounded-full flex items-center gap-1 border border-red-200">
            {AGENTS.muscle.icon} {AGENTS.muscle.name}
          </span>
          <span className="text-xs font-medium bg-slate-100 text-slate-800 px-2.5 py-0.5 rounded-full flex items-center gap-1 border border-slate-200">
            {AGENTS.gloomy.icon} {AGENTS.gloomy.name}
          </span>
        </div>
      </header>
